...
这是当前平台基础里程碑的本地使用说明。当前版本已经具备 FastAPI 健康服务、Next.js 三栏可调整宽度的学习工作区、PostgreSQL/pgvector、带密码认证的 Redis、MinIO、Docker 服务拓扑，以及自动化测试和 CI。用户可注册、登录、退出，拥有独立个人空间；任意已登录用户可创建班级，并通过邀请码邀请学生加入。计费和知识入库属于后续里程碑，当前版本尚未提供。
